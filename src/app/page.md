...
| getCountFromServer(collection(db |
| setStats((prev) => ({ ...prev |
| ) |
| getCountFromServer(collection(db |
| setStats((prev) => ({ ...prev |
...
| } |
| const toursCount = useCountUp(Number(stats.tours.replace("+" |
| const hotelsCount = useCountUp(Number(stats.hotels.replace("+" |
